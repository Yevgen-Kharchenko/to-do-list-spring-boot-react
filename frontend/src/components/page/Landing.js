...
export default function Landing({isAuthenticated, setIsAuthenticated}) {
  ...
  useEffect(() => {
    async function getAndSetNumberAllTodo() {
      try{
        const response = await axios.get('http://13.60.64.164:3001/api/todo/count', {
          headers: {
    ...
    async function getAndSetNumberAllTodoNotCompleted() {
      try{
        const response = await axios.get('http://13.60.64.164:3001/api/todo/count?isCompleted=false', {
          headers: {
            'Authorization': `Bearer ${sessionStorage.getItem('token')}`,
          }
        });
